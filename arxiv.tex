...
For this section and rest of the article, we consider the conditoinal probability of $A$ given $B$, notated as $P(A | B)$ to be zero whenever $B$ is a measure zero event.
...
\iffalse 
\begin{definition}[Selective dominance]
    Consider a p-value $p$ for the null $H_0$ that is valid given $Z$ as in \eqref{eq:valid_given_z}. We say that $p$ is \textbf{selectively dominant given $Z$} if for any fixed distribution $P$ under the null $H_0$ and any selection function $s(x, z)$ for which $p$ and $U$ both have positive probability of selection, i.e., 
    \begin{equation*}
        P(S = 1), P(S'=1),
    \end{equation*}
    $p$ has a conditional PDF given $Z$ and 
    \begin{equation}
    p \mid Z, S = 1 \succeq U \mid Z, S' = 1
    \end{equation}
    meaning for all $t \in \R$ that 
    \begin{equation*}
        P(p \leq t | Z, S=1) \leq P(U \leq t | Z, S'=1)
    \end{equation*}
    where the inequality is almost sure. 
\end{definition}
\fi 
