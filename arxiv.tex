...
\end{equation*}
which satisfies $F_{U|z, S' =1}(F^{-1}_{U |z, S'=1}(t)) = t$ and $F_{U | z, S' = 1}(x) \leq t \iff x \leq  F^{-1}_{U | z, S' = 1}(t)$. By \Cref{lem:pos_selection} $F_{U|Z S'=1}(t)$ will be a continuous function of $x$ with probability one both under $Q$ and $Q'$. The following string of equalities and inequalities then hold almost surely under $Q$:
...
\end{equation*}
Writing the event $Q'(U \leq F_{U |Z, S' = 1}^{-1}(t) |Z) = t$ as $Z \in A$ for some Borel $A$, \Cref{lem:switch} tells us that $Q'(A) = 1 \implies Q(A) = 1$, which establishes the claim. 
