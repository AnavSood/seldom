...
\end{equation}
The random vector $Z$ plays an integral role in establishing the relationship between $p$ and a binary selection random variable $S \in \{0, 1\}$, which takes value one when $p$ is selected and zero otherwise. The relationship between $p$, $Z$, and $S$ is governed by a \textbf{selection function} \footnote{Formally, the selection function $s(\cdot, \cdot)$ such is an instantiation of the conditional expectation $s(p, Z) = E[S|p , Z ]$. },
\begin{equation*}
