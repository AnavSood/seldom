...
# Hack to get __version__ from seldom/__init__.py
with open("seldom/__init__.py") as f:
    for line in f:
...
include_dirs = [
    os.path.join("seldom", "src"),
    os.path.join("seldom", "src", "include"),
]
extra_link_args = []
libraries = []
library_dirs = []
...
# check if micromamba environment activated (CI)
    ...
    conda_prefix = os.path.join(os.environ["MAMBA_ROOT_PREFIX"], "envs", "seldom")
else:
    conda_prefix = None
...
ext_modules = [
    Pybind11Extension(
        "seldom.seldom_core",
        sorted(glob("seldom/src/*.cpp")),  # Sort source files for reproducibility
...
# Include seldom and all submodules.
# This removes setuptools warning about missing namespace packages.
packages = ["seldom"] + [
    f"seldom.{submod}"
    for submod in find_namespace_packages("seldom")
]

setup(
    name='seldom', 
    version=__version__,
    description='Selective inference using p-values.',
    long_description='',
    author='Anav Sood',
    author_email='anavsood@gmail.com',
    maintainer='Anav Sood',
    maintainer_email='anavsood@gmail.com',
    packages=packages,
    package_data={
        "seldom": [
            "src/**/*.hpp",
